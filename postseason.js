...
(() => {
  ...
  function syncNavigation() {
    $('playoffsNav')?.classList.toggle('hidden',!live());
  }
  ...
  function funnel(data) {
    ...
    return '<ol class="ps-funnel" aria-label="Postseason progression">'+[[8,'Wild Card'],[6,'Divisional'],[4,'Conference'],[2,'Super Bowl'],[1,'Champion']].map(([n,title],i)=>'<li class="'+(data.settings.status==='complete'?i===4?'ps-stage-current':'ps-stage-done':i===current-1?'ps-stage-current':i<current-1?'ps-stage-done':'')+'"><b>'+n+'</b><span>'+title+'</span></li>').join('')+'</ol>';
  }
  ...
  function renderPage() {
    ...
    try {
      ...
      const box=$('playoffBox'), fingerprint=JSON.stringify([data.settings,data.entries,data.current,data.history]);
      if(box.dataset.fingerprint!==fingerprint) {
        ...
        box.innerHTML=data.settings.status==='scheduled'?'<div class="card"><h2>Postseason is scheduled for January</h2><p>Regular-season scoring is still in effect. The commissioner will freeze standings and start the Wild Card card during its scheduled setup week.</p>'+button('View season calendar','calendar')+'</div>':champion(data)+funnel(data)+personal(data)+raceTable(data)+eliminated(data)+history(data);
  ...
  async function renderCommissioner() {
    ...
    if(!scheduled&&data.current?.round.status==='finalized'&&data.current.round.round_number<4)setup+=button('Create '+(['','Wild Card','Divisional','Conference Championship','Super Bowl Championship'][data.current.round.round_number+1]),'create-round');
  ...
  async function handle(action,b) {
    ...
    if(action==='start'){
      if(!preview)return;if(!confirm('Freeze these regular-season points and seeds, then open the Wild Card card?'))return;
    ...
    if(action==='preview-round'){
      ...
      $('postseasonRoundPreview').innerHTML=raceTable(data,current)+(current.ready?button('Finalize round · '+(current.round.participants_after===1?'crown champion':'eliminate 2'),'finalize','',false):'<p class="notice">Enter all scored results and the actual tiebreaker, then calculate again.</p>');return;
    }
    if(action==='finalize'){
