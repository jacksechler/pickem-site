...
(() => {
  ...
  if (!M) return;
  const client = new M.Client(), boards = new Map(), latest = new Map();
  ...
  function card(q, item) {
    ...
    return '<article class="ls-game' + (g.state === 'live' ? ' ls-in-progress' : '') + '"><div class="ls-game-top"><span class="ls-sport">' + esc(M.leagues[g.league].label) + '</span><span class="ls-status">' + esc(status) + '</span></div><div class="ls-teams">' + g.teams.map(t => '<div class="ls-team">' + (t.logo ? '<img src="' + esc(t.logo) + '" alt="" width="30" height="30" loading="lazy" referrerpolicy="no-referrer">' : '<span class="ls-logo-fallback" aria-hidden="true">' + esc(t.abbreviation) + '</span>') + '<span>' + esc(t.name) + '</span><strong>' + (t.score == null ? '—' : t.score) + '</strong></div>').join('') + '</div><div class="ls-game-bottom"><span>' + (item.stale || (item.checkedAt && Date.now() - item.checkedAt > 180000) ? 'Update delayed' : g.state === 'final' ? 'Final score' : g.state === 'live' ? 'In progress' : g.state === 'paused' ? 'Schedule update' : 'Upcoming') + '</span><a href="' + esc(g.url) + '" target="_blank" rel="noopener noreferrer">ESPN <span aria-hidden="true">↗</span><span class="gd-sr">: ' + esc(g.name) + '</span></a></div></article>';
  }
  ...
  async function saveLink(questionId, game) {
    ...
    const payload = {espn_event_id: game?.id || null, espn_league: game?.league || null, espn_event_date: game?.day || null};
  ...
  async function managerAction(button) {
    ...
    try {
      if (action === 'match') {
        ...
        const feeds = await Promise.all(Object.keys(M.leagues).map(key => client.readBoard(key, range.from, range.to)));
